...
- **Confirmations, trois niveaux** — le geste choisit son niveau, pas l'écran :
  ...
  Sur **mobile**, le pied d'un dialog `danger` ne remonte **pas** l'action principale : `.dialog-foot` y passe en `column` au lieu du `column-reverse` général, pour que l'action irréversible ne se retrouve jamais au-dessus de la sortie sûre. Rien à écrire dans la vue — la classe `danger` du composant porte la règle.
