...
- **Effet immédiat** : statut séance `cancelled`, **libère le quota** de tous les `participating` (déclenche mécanisme B §4.10), inscriptions passent en `cancelled` (soft flag), notif aux inscrits **toujours envoyée** — l'annulation n'offre **pas le choix de notifier ou non**, contrairement à la modification (§ ci-dessus) : l'événement est trop structurant. À ne pas confondre avec la case d'accusé de réception ci-dessus, qui arme le bouton sans jamais conditionner l'envoi, `AuditLog cancel_session`.
...
- **Notifications** : celles déjà reçues restent **lisibles** dans les cloches — titre et créneau y sont figés — mais **ne renvoient plus** vers la séance. Une notification a réellement été envoyée : l'effacer réécrirait l'histoire, la laisser pointer le vide mènerait à une page morte. Celles **encore en file** partent quand même, en renvoyant vers le planning : quelqu'un qui vient d'apprendre l'annulation doit la recevoir. Le dialog **chiffre les deux séparément**, la cloche et la file n'étant ni le même volume ni le même effet.
