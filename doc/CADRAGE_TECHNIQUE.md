...
- **Pas de bibliothèque** (`spatie/icalendar-generator` écartée) : le sous-ensemble utile de la RFC 5545 tient en une classe (`app/Support/Ics.php`) — échappement TEXT, pliage à 75 octets sans couper un caractère UTF-8, VEVENT, VALARM. Une dépendance de plus serait une surface de maintenance pour un bénévole solo, sans gain.
...
- **Route publique** `GET /agenda/{jeton}.ics`, hors `auth`, sous `throttle:600,1` : la limite compte par IP, et les agendas en ligne relisent tous leurs abonnés depuis quelques serveurs partagés — une limite basse priverait de mise à jour les abonnés d'un même fournisseur. Le jeton de 64 caractères rend l'énumération hors de portée sans l'aide du throttle. `REFRESH-INTERVAL` et `X-PUBLISHED-TTL` à 1 h.
